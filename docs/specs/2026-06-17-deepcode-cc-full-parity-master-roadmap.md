...
- **TUI 计划3（会话+小件）✅ 已完成并合 main**：1.6 Task 依赖图薄片（blocks/blockedBy + in_progress 门控 + 软删依赖视同已清 + TaskUpdate addBlocks/addBlockedBy）+ 1.7 Hooks 进度（runHooks 慢阶段 onProgress[热事件不触发]→Spinner hookLabel，双接线）+ 5.7 /statusline（statusLineCommand[DANGEROUS 剥离 project]+事件驱动去抖 runner[300ms/在途abort/5s/失败保留缓存绝不抛，对齐 CC StatusLine.tsx]+StatusFooter 段）+ 3.6 /fork+/rename（SessionMeta.title+appendTitle，/fork=CC /branch 共享 memdir 对齐 CC+(Branch) 标题）。9 任务 SDD（每任务 sonnet 双审 + opus 全分支终审 Ready）+ 真机冒烟过（/rename+/fork+/resume、statusLine 渲染+flag 信任边界、Stop hook 进度）。**2 个 CC 实读校准**：/fork memdir 共享不隔离（推翻旧 forked 隔离判断）、statusline 事件驱动+去抖（非 interval）。spec/plan `docs/{specs,plans}/2026-06-24-deepcode-tui-batch-3-session-misc*`。follow-up（非阻塞）：execStatusLineCommand 直接单测、/fork 可选拷贝 task/checkpoint、/compact 在 GLM 下卡=provider 层（与本批无关）。
**🎯 下一步=新会话与用户确认**：1.4✅/5.9✅/2.7✅/5.3✅/5.4✅/3.6✅/5.7✅/1.7✅/**1.5✅(2026-06-25 合 main)**/3.7 /permissions UI✅ 已完成；**剩 1.6 重型 Team/teammate（归第6层）、5.1 keybindings（L）、SKIP 项（4.11/5.2/5.5/5.10/5.11）**。或回捡 C 类推迟件（3.8 Sandbox/3.10 Cron/4.6 LSP/4.2 ToolSearch 按 ROI）/第6层云平台（进前确认是否启动自建云）。或回捡 C4 推迟件（LSP/Cron/Sandbox）/第6层云平台。教训沉淀：①TUI 双组件接线必双改 App+FullscreenApp（[[deepcode-tui-dual-component]]）②冒烟 flag 文件别放 harness scratchpad（/private/tmp/claude-501/... 沙箱私有用户终端读不到→放家目录）③纯逻辑功能先 tsx 诊断脚本证明对，再排查冒烟环境问题。
...
| 1.5 | **可写 subagent + git worktree**（EnterWorktree/ExitWorktree + isolation 字段 + 类型级 deny） | `tools/EnterWorktreeTool` `utils/worktree.ts` | ✅ **完成**（2026-06-25 SDD 7 任务+冒烟修，真·照搬 CC：三遍实读 CC v2.1.76 核实。`src/worktree.ts` git底座；**删 MAX_ACTIVE 信号量**靠 loop CONCURRENCY=5 每级独立批=CC 每 query 独立调度器无死锁；`GLOBAL_SUBAGENT_DENY=['ExitPlanMode','EnterWorktree','ExitWorktree']` 子代理默认可写+递归（池 Agent 自引用）、Explore/Plan 靠 disallowedTools 仍只读；子代理独立 cwd+worktreePath+隔离系统提示；Agent `isolation:"worktree"`(无改动删/有改动留回传 path+branch,前后台 abort 清理)；EnterWorktree/ExitWorktree 会话级工具+worktreeSession;settings.worktree 全层生效+symlink/sparse+非 git WorktreeCreate/Remove hook 兜底(hookBased)。opus 全分支终审 Ready-to-merge；真机冒烟 PASS(EnterWorktree 切 cwd+页脚 live+symlink+ExitWorktree 恢复)。裁决：Agent isReadOnly:true 保持(承载 loop 并发批+plan 门控)。spec/plan `docs/{specs,plans}/2026-06-25-deepcode-1.5-writable-subagent-worktree*`） | ✅ | 碰TUI | L | L-040✅ |
| 1.6 | **多 agent 工作流**（TeamCreate/TeamDelete + TaskUpdate 依赖图 blocks/blockedBy + 成员寻址） | `tools/TeamCreateTool` `tools/TaskUpdateTool` | 🟡 **依赖图薄片已完成**（TUI 计划3 merge：Task blocks/blockedBy + in_progress 门控 + 软删依赖视同已清 + TaskUpdate addBlocks/addBlockedBy）；Team/teammate/SendMessage 重型件仍 ⬜（归第6层） | 🟡 | 碰TUI | L | 1.3 |
| 1.7 | **Hooks TUI 进度（①e）** | `hooks/` 渲染 | ✅ **完成**（TUI 计划3 merge：runHooks 慢阶段 onProgress[热事件不触发]→Spinner hookLabel「正在运行 X 钩子…」，App+FullscreenApp 双接线；真机冒烟过 Stop 钩子） | ✅ | 碰TUI | M | L-042✅ |
...
- **7.1 Workflow 编排 DSL**（最大空白）✅ **已完成（merge `655b3ca`，2026-06-30，阶段 B 起点）**：内置工具 `Workflow`，用**确定性 JS 脚本**编排多 subagent（`agent()`/`parallel()`/`pipeline()`/`phase()`/`log()`/`budget`/`workflow()`），跑在 Node vm 沙箱（禁 `Date.now`/`Math.random`/`import()` 保可 resume，async 蹦床桥接），完成的 agent() 调用按**结构化路径键(AsyncLocalStorage)**做快照可 `resumeFromRunId` 增量重跑；`/workflows` 进度树；`ultracode` 触发；`workflow()` 一层嵌套。src/workflow/{parse,sandbox,backend,journal,runtime,orchestrator,resolve,types}.ts + tools/workflow.ts + tui/WorkflowView.tsx。12 任务 SDD + 4「全部连线」fix + opus 全分支终审 + 真机冒烟(glm-5.2 端到端,揪修 /workflows 容错+TaskOutput 回退 result 2 bug)。spec/plan docs/{specs,plans}/2026-06-29-deepcode-workflow*。
