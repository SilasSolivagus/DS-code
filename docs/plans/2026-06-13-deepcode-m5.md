...
**执行注意:** 12 个任务建议分 3 个执行会话（Task 1-4 基础设施与核心 hook / Task 5-9 组件 / Task 10-12 接线与验收）。每任务双门（规格审查 + 质量审查）。**ink 组件测试约定**：用 ink-testing-library 的 `render()` + `lastFrame()` 断言渲染输出、`stdin.write()` 模拟按键；测试文件后缀 `.test.tsx`。
