...
- Modify: `src/config.ts`（Settings 接口加 `provider?`/`providers?` 字段 + import `type CustomProvider`——**仅类型，使 Task 1 能单独 typecheck**；parser 留 Task 2）
...
- Produces:
  ...
  - `interface CustomProvider { baseURL: string; apiKeyEnv?: string; apiKey?: string; dialect?: Dialect; models: { fast: string; smart: string }; meta?: Record<string, ModelMeta>; defaultMeta?: ModelMeta }`
...
- **类型循环说明**：providers.ts `import { loadSettings } from './config.js'`（值）+ `import type { Settings } from './config.js'`（类型）；config.ts `import type { CustomProvider, ModelMeta } from './providers.js'`（仅类型）。类型循环 TS 编译期可解，运行时无循环（config.js 不在运行时 import providers.js）。`CustomProvider` 由 providers.ts 拥有（Task 2 不再重复定义）。
...
并编辑 `src/config.ts` 让 `Settings` 类型带上新字段（**仅类型，parser 留 Task 2**）：
...
> 此处只加类型字段，使 `resolveActiveProvider(settings)` 能读 `settings.provider`/`settings.providers` 并通过 typecheck。运行时解析（`parseProvidersConfig` + `loadRawUserSettings` 接线）在 Task 2。
...
```bash
npm run typecheck
git add src/providers.ts src/config.ts test/providers.test.ts
...
## Task 2: config.ts providers 运行时解析

**Files:**
- Modify: `src/config.ts`（新增 `parseProvidersConfig`；`loadRawUserSettings` 接线）
- Test: `test/config.providers.test.ts`

**Interfaces:**
- Consumes: `CustomProvider`/`ModelMeta` 类型（已在 Task 1 由 providers.ts 定义并被 config.ts import）；Settings 的 `provider`/`providers` 字段（Task 1 已加）。
- Produces:
  - `function parseProvidersConfig(raw: unknown): Settings['providers'] | undefined`
  - `loadRawUserSettings` 返回对象带 `provider`/`providers`（写路径/raw 读路径生效）。

> 注：`CustomProvider` 已在 Task 1 的 providers.ts 定义、config.ts 已 `import type { CustomProvider }`。Task 2 **不再重复定义** CustomProvider，只加运行时 parser。若 parser 内需 `ModelMeta` 类型，config.ts 顶部补 `import type { ModelMeta } from './providers.js'`。
...
在 `src/config.ts`（`Settings.provider`/`providers` 字段与 `import type { CustomProvider }` 已在 Task 1 加好；本步只加 parser + 接线）：

文件顶部 import 区按需补（若 parser 用到 `ModelMeta`）：
