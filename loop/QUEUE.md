...
### auto/slash-keybindings — `/keybindings` 查看快捷键 · S · await-review:smoke
- **gap：** L-005
- **做了什么：** 新增 `/keybindings`，分组静态展示真实快捷键（输入框 / 滚动 / 触发 / 选中）。键位逐条对照 `InputBox.tsx`/`FullscreenApp.tsx` 源码核实，无编造。纯函数 `src/keybindings.ts`，useChat 接线、suggest/help 同步。
- **怎么验：**
  - `git checkout auto/slash-keybindings && npm test && npm run typecheck`（应 280 绿 / 干净）
  - 真机冒烟：`npm start` → 打 `/` 看菜单里**有 `/keybindings`** → 跑 `/keybindings` → 核对列出的键位与实际操作一致（PageUp/PageDown 滚动、Ctrl+G 贴底、Esc 中断/清空、/ @ ! 触发等）。
- **合：** `git checkout main && git merge --no-ff auto/slash-keybindings`（合后 BACKLOG L-005 标 `merged`、删本条）
- **丢：** `git branch -D auto/slash-keybindings`（BACKLOG L-005 标回 `todo`）
