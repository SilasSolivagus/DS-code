...
export function App(props: {
  ...
  // 加 1 个页脚顶空行（StatusFooter 外层 marginTop=BLOCK_GAP=1；簇间空行已去）= 1，
  // 加底边线(1) + ink 末尾换行(1) = 6；再加按需行：记忆行 + 工具行 + statusLine行。
  const linesBelowCaret = 6 + (memoryCount > 0 ? 1 : 0) + (toolCounts.length > 0 ? 1 : 0) + (state.statusLineOutput ? 1 : 0)
  useEffect(() => {
